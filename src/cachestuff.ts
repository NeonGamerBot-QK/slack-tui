...
export async function compileRequestedChannels(channels: string[], app: App) {
  const currentChannelCache = fs.existsSync("assets/data/cache-channels.json")
    ? JSON.parse(fs.readFileSync("assets/data/cache-channels.json").toString())
    : [];
  const newStuff = [];
  ...
  fs.writeFileSync(
    "assets/data/cache-channels.json",
    JSON.stringify([...newStuff, currentChannelCache]),
  );
}

export function getCachedChannels() {
  return JSON.parse(
    fs.readFileSync("assets/data/cache-channels.json").toString(),
  );
}
